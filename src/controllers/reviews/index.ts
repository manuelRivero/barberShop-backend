...
export const createReview = async (
  ...
  try {
    ...
    const targetUser = await User.findById(new mongoose.Types.ObjectId(barber))

    if (!targetUser) {
      res.status(404).json({ ok: false, message: "El barbero no fue encontrado" })
    } else {

      targetUser.score = (targetUser?.score + score) / 2
